...
public class Board
{
    ...
    {
        List<NormalItem.eNormalType> allTypes = Enum.GetValues(typeof(NormalItem.eNormalType)).Cast<NormalItem.eNormalType>().ToList();
        List<NormalItem.eNormalType> result = new List<NormalItem.eNormalType>();
        int cellsToFill = (boardSizeX * boardSizeY / m_matchMin) * m_matchMin;

        foreach (NormalItem.eNormalType type in allTypes)
        {
            if (result.Count + m_matchMin > cellsToFill) break;
        ...
        while (result.Count < cellsToFill)
        {
            NormalItem.eNormalType type = allTypes[UnityEngine.Random.Range(0, allTypes.Count)];
            for (int i = 0; i < m_matchMin && result.Count < cellsToFill; i++)
            {
                result.Add(type);
            }
        }
        ...
        {
            int rnd = UnityEngine.Random.Range(i, result.Count);
    ...
    public IEnumerable<Cell> Cells
    {
        get
        {
            for (int x = 0; x < boardSizeX; x++)
            {
                for (int y = 0; y < boardSizeY; y++)
                {
                    if (m_cells[x, y] != null) yield return m_cells[x, y];
                }
            }
        }
    }
    ...
    public Cell GetFirstOccupiedCell()
    {
        return Cells.FirstOrDefault(cell => !cell.IsEmpty);
    }

    public Cell GetFirstOccupiedCellOfType(NormalItem.eNormalType type)
    {
